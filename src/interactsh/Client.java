...
public class Client {
    ...
    // Mainly use default values for Client
    // Scheme is whether to use TLS or not
    public Client(String Host, boolean Scheme, String Authorization, int Port) {
        host = Host;
        scheme = Scheme;
    ...
    public ArrayList<InteractEntry> poll() throws IOException, InterruptedException {
        ArrayList<InteractEntry> interactions = new ArrayList<>();
        ...
        try {
            ...
            if (!jsonObject.isNull("data")) {
                ...
                for (int i = 0; i < data.length(); i++) {
                    ...
                    interactions.add(new InteractEntry(decryptedData));
                }
            }
        } catch (Exception ex) {
            burp.BurpExtender.api.logging().logToError(ex.getMessage());
