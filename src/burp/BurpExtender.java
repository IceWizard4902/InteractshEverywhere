...
class Monitor implements Runnable, IExtensionStateListener {
    ...
    private void processInteraction(InteractEntry interaction) {
        String id = interaction.uid;
        Utilities.out("Got an interaction:" + interaction);
...
class Correlator {
    ...
    Correlator() {
        ...
        client = new Client("oast.pro", true, null, 443);
        client_ips = new HashSet<>();

        try {
            client.generateKeys();
            client.registerClient();
            String pollPayload = client.getInteractDomain();
            Utilities.callbacks.makeHttpRequest(pollPayload, 80, false, ("GET / HTTP/1.1\r\nHost: " + pollPayload + "\r\n\r\n").getBytes());
            for (InteractEntry interaction: client.poll()) {
                client_ips.add(interaction.address);
            }
